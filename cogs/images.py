    ...
    async def e621(self, ctx, *, tags: str):
        ...
        if filters:
            for t in filters:
                if t['filterName'].lower().strip() == "e621":
                    blacklist = t['result'].replace(" ", "").replace(",", " ")
        ...
        try:
            ...
            while (retry < 6):
                ...
                if not [i for e in blacklist.split(" ") for i in rand_image_tags.split(" ") if e in i]:
                    ...
                    if retry < 5:
                        ...
                        await ctx.send("Sorry, all results used one or more blacklisted tag.{} {}".format(ctx.message.author.mention, blacklist) )
                        break
